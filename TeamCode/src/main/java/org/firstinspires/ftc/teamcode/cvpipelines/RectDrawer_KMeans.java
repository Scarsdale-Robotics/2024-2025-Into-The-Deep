...
public class RectDrawer_KMeans extends OpenCvPipeline {
...
//    }
    ...
    public RectDrawer_KMeans(Telemetry telemetry){
...
//        Imgproc.erode(inRange, inRange, kernel);
        ...
        // Convert the image to float32 for K-means
        Mat imageFloat = new Mat();
        image.convertTo(imageFloat, CvType.CV_32F);

        // Flatten the image to a 2D matrix of pixels (rows = pixels, cols = channels)
        Mat reshapedImage = imageFloat.reshape(1, image.rows() * image.cols());

        // Define the criteria for K-means (Stopping criteria: max iterations or epsilon threshold)
        TermCriteria criteria = new TermCriteria(TermCriteria.COUNT + TermCriteria.EPS, 100, 0.2);

        // Number of clusters (segments)
        int km = 3; // You can change this based on how many segments you want

        // Set the initial labels for each point (randomly initialized)
        Mat labels = new Mat();

        // Run K-means clustering
        Mat centers = new Mat(); // To store the cluster centers
        Core.kmeans(reshapedImage, km, labels, criteria, 10, Core.KMEANS_RANDOM_CENTERS, centers);

        // Convert labels back to a usable format and reshape the image
        labels = labels.reshape(1, image.rows());  // Reshape labels to match the original image's row count
        Mat segmentedImage = new Mat(image.size(), image.type());

        // Assign color to each cluster based on the center
        for (int i = 0; i < image.rows(); i++) {
            for (int j = 0; j < image.cols(); j++) {
                int clusterIdx = (int) labels.get(i, j)[0];  // Get the cluster index for each pixel
                Scalar color = new Scalar(centers.get(clusterIdx, 0));  // Get the color of the cluster center
                segmentedImage.put(i, j, color.val);  // Assign the pixel to the color of the cluster
            }
        }

        if (true) return segmentedImage;
