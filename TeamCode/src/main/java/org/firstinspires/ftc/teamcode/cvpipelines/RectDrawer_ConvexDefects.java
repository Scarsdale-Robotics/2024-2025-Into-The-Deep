...
public class RectDrawer_ConvexDefects extends OpenCvPipeline {
...
//    }
    ...
    public RectDrawer_ConvexDefects(Telemetry telemetry){
...
//        Imgproc.erode(inRange, inRange, kernel);
        ...
        for (MatOfPoint contour : contours) {
            ...
            if (contour.rows() > 3) {
                Imgproc.convexityDefects(contour, hull, defects); // Calculate convexity defects
            }

            // Visualize the defects (highlight the defects on the contour)
            for (int i = 0; i < defects.rows(); i++) {
                // Get the defect points
                int startIdx = (int) defects.get(i, 0)[0];
                int endIdx = (int) defects.get(i, 1)[0];
                int farIdx = (int) defects.get(i, 2)[0];
                int depth = (int) defects.get(i, 3)[0];

                // Get the points from the contour
                Point start = contour.toList().get(startIdx);
                Point end = contour.toList().get(endIdx);
                Point far = contour.toList().get(farIdx);

                // Draw the start and end points of the defect
                Imgproc.line(binaryImage, start, end, new Scalar(0, 255, 0), 2);  // Green line
                Imgproc.circle(binaryImage, far, 5, new Scalar(0, 0, 255), -1);  // Red circle

                // Optionally, filter the defects based on depth
                if (depth > 30) {  // Example: Only keep significant defects
                    Imgproc.line(binaryImage, start, far, new Scalar(255, 0, 0), 2);  // Blue line
                }
            }
        }

        if (true) return binaryImage;
