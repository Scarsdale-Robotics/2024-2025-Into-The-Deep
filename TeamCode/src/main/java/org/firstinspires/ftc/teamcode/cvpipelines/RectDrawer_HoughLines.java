...
public class RectDrawer_HoughLines extends OpenCvPipeline {
...
//    }
    ...
    public RectDrawer_HoughLines(Telemetry telemetry){
...
//        if (true) return inRange;
        ...
        for (MatOfPoint contour : unfilteredContours) {
            ...
            if (area > minArea && area < maxArea) {
                ...
                telemetry.addData("AREA GREATER THAN MAX", "!!!");
                List<MatOfPoint> brokenContours = breakLargeContour(inRange, contour);
                telemetry.addData("brokenContours.size()", brokenContours.size());
                telemetry.update();
        ...
        for (ArrayList<Double[]> overlapGroup : overlapGroups) {
            int maximumIndex = overlapGroup.get(0)[0].intValue();
            double maximumArea = overlapGroup.get(0)[1];
            for (Double[] rect : overlapGroup) {
                if (rect[1] > maximumArea) {
                    maximumArea = rect[1];
                    maximumIndex = rect[0].intValue();
                }
            }
            filteredRects.add(rotatedRects.get(maximumIndex));
        }
...
//        }
        ...
        if (!filteredRects.isEmpty()) {
            int n = 0;
            telemetry.addData("width ", filteredRects.get(n).size.width);
            telemetry.addData("height ", filteredRects.get(n).size.height);
            telemetry.addData("area ", filteredRects.get(n).size.area());
            telemetry.addData("angle ", filteredRects.get(n).angle);
            telemetry.addData("center ", filteredRects.get(n).center);
...
//            output.add(new Point((i.center.x - 320) / 640 * canvasHorizontal, -(i.center.y - 240) / 240 * canvasVertical));
            double procAngle = filteredRects.get(n).angle;
            if (filteredRects.get(n).size.width > filteredRects.get(n).size.height)
                procAngle *= -1;
            else
